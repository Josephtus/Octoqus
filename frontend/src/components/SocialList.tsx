  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto">
      <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[32px] shadow-2xl relative overflow-hidden flex flex-col">
        ...
        {/* Header Section */}
        <div className="p-5 sm:p-8 relative z-10 border-b border-white/5">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-8">
            <div>
              <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tighter">Sosyal Ağ</h2>
              <p className="text-slate-500 text-[10px] sm:text-xs font-bold uppercase tracking-widest mt-1">Octoqus kullanıcıları ile bağlantı kur</p>
            </div>
            ...
            <div className="relative w-full max-w-md group">
              ...
              <input 
                ...
                className="w-full pl-12 sm:pl-14 pr-4 sm:pr-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-slate-950/50 border border-white/5 text-white text-sm focus:outline-none focus:border-[#00f0ff]/50 transition-all"
                value={search}
        ...
        {/* Results Section */}
        <div className="p-4 sm:p-6 md:p-8 min-h-[200px] flex flex-col justify-center relative z-10">
                  ...
                  {users.filter(u => u.id !== currentUserId).map((user, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 hover:border-[#00f0ff]/30 transition-all group"
                      >
                        <div className="flex items-center gap-3 sm:gap-4 cursor-pointer flex-1" onClick={() => setSelectedUser(user)}>
                          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-slate-800 overflow-hidden border border-white/10 shadow-lg shrink-0">
                            {user.profile_photo ? (
                           ...
                           <div>
                            ...
                            <p className="text-slate-500 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">{user.mail}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          {status === 'ACCEPTED' ? (
                            <button 
                              ...
                              className="w-full sm:w-auto px-4 sm:px-6 py-2.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 bg-white/5 text-white border border-white/10 hover:bg-red-500/10 hover:text-red-500"
                            >
                            ...
                            isSender ? (
                              <button 
                                onClick={() => handleFriendAction(user.id, 'remove')}
                                className="w-full sm:w-auto px-4 sm:px-6 py-2.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 bg-white/5 text-slate-400 border border-white/5 hover:text-red-500"
                              >
                              ...
                              <div className="flex items-center gap-2 w-full">
                                <button 
                                  ...
                                  className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 bg-[#00f0ff] text-slate-950"
                                >
                                  Onayla
                                </button>
                                <button 
                                  onClick={() => handleFriendAction(user.id, 'decline')}
                                  className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 bg-white/5 text-white border border-white/10"
                                >
                                  Reddet
                                </button>
                              </div>
                            ...
                            <button 
                              ...
                              className="w-full sm:w-auto px-4 sm:px-6 py-2.5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 bg-[#00f0ff] text-slate-950 shadow-lg shadow-[#00f0ff]/20 hover:scale-105"
                            >
