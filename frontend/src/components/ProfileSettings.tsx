...
export const ProfileSettings: React.FC<ProfileSettingsProps> = ({ onUpdate }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-fade-in pb-20">
      {/* Profil Header Card */}
      <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[24px] sm:rounded-[40px] p-6 sm:p-8 md:p-12 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
          ...
          <div className="relative group">
            <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-[32px] sm:rounded-[48px] bg-slate-800 border-4 border-white/10 overflow-hidden shadow-2xl transition-transform group-hover:scale-105 duration-500 relative">
              {user?.profile_photo ? (
                <>
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-4">
              <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tighter">{user?.name} {user?.surname}</h2>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
              <button 
                ...
                className={`flex-1 sm:flex-none justify-center flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${
              ...
              <button 
                ...
                className={`w-full sm:w-auto justify-center flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'settings' ? (
          ...
          >
            {/* Profil Form Card */}
            <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[24px] sm:rounded-[40px] p-6 sm:p-8 md:p-12 shadow-2xl">
              <form onSubmit={handleSubmitProfile(onProfileSubmit)} className="space-y-10">
                ...
                <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row gap-6 md:items-center justify-between">
                  ...
                  <button 
                    type="submit"
                    disabled={saving}
                    className="w-full md:w-auto px-8 sm:px-12 py-4 sm:py-5 bg-white text-slate-950 rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest hover:bg-[#00f0ff] hover:shadow-[0_0_40px_rgba(0,240,255,0.3)] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                  >
              ...
              <form onSubmit={handleSubmitPassword(onPasswordSubmit)} className="space-y-10">
                ...
                <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row gap-6 md:items-center justify-between">
                  ...
                  <button 
                    type="submit"
                    disabled={pwSaving}
                    className="w-full md:w-auto px-8 sm:px-12 py-4 sm:py-5 bg-white/5 text-white border border-white/10 rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest hover:bg-[#b026ff] hover:border-[#b026ff] hover:shadow-[0_0_40px_rgba(176,38,255,0.3)] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                  >
          ...
          <motion.div 
            key="social"
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[24px] sm:rounded-[40px] p-4 sm:p-8 md:p-12 shadow-2xl min-h-[500px]"
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {socialData.map((item, idx) => (
                  <motion.div 
                    ...
                    className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all group"
                  >
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      {activeTab === 'friends' ? (
                        <button 
                          ...
                          className="w-full sm:w-auto px-4 py-2.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all flex justify-center items-center"
                        >
                          Arkadaştan Çıkar
                        </button>
                      ) : (
                        <div className="flex items-center gap-2 w-full">
                          <button 
                            ...
                            className="flex-1 sm:flex-none px-4 py-2.5 bg-[#00f0ff]/10 text-[#00f0ff] border border-[#00f0ff]/20 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-[#00f0ff] hover:text-slate-950 transition-all flex justify-center items-center"
                          >
                            Onayla
                          </button>
                          <button 
                            onClick={() => handleFriendAction(item.id, 'decline')}
                            className="flex-1 sm:flex-none px-4 py-2.5 bg-white/5 text-white border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex justify-center items-center"
                          >
                            Reddet
                          </button>
                        </div>
                      ...
                      <button 
                        ...
                        className="hidden sm:block p-2 bg-white/5 text-slate-400 hover:text-white rounded-xl transition-all"
                      >
