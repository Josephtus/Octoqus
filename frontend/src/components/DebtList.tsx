  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10">
      <AnimatePresence>
        {myActionNeeded.length > 0 && (
          <motion.div 
            ...
            className="bg-amber-500/10 border border-amber-500/20 rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-8 backdrop-blur-xl"
          >
            ...
            <div className="space-y-3">
              {myActionNeeded.map(s => (
                <div key={s.id} className="bg-slate-900/60 border border-white/5 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button 
                      ...
                      className="flex-1 sm:flex-none px-4 py-2 bg-red-500/10 text-red-500 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all disabled:opacity-50"
                    >
                      REDDET
                    </button>
                    <button 
                      ...
                      disabled={actionLoading === s.id}
                      className="flex-1 sm:flex-none px-4 py-2 bg-emerald-500 text-slate-950 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-400 transition-all disabled:opacity-50"
                    >
                      ONAYLA
                    </button>
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-slate-900/40 border border-white/5 p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] backdrop-blur-3xl shadow-xl relative overflow-hidden group">
      ...
      <div className="bg-slate-900/40 border border-white/5 rounded-[1.5rem] sm:rounded-[3rem] p-4 sm:p-8 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          {myTransactions.map((tx, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="group relative flex flex-col gap-4 p-4 sm:p-6 bg-slate-950/40 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] hover:bg-slate-800/40 hover:border-white/10 transition-all duration-500 shadow-sm"
              >
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                   <div className="flex items-center gap-3 px-4 sm:px-6 py-3 sm:py-4 bg-slate-950/50 rounded-2xl border border-white/5 group-hover:border-white/10 transition-all">
                   ...
                   {isIDebt && (
                      <div className="flex items-center gap-2">
                        {hasPendingRequest ? (
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 px-3 sm:px-6 py-3 sm:py-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl">
                               <div className="flex flex-col">
                          ...
                          <button 
                            ...
                            className="w-full sm:w-auto px-6 sm:px-10 py-3 sm:py-4 bg-[#00f0ff] text-slate-950 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all shadow-xl hover:scale-105 hover:shadow-[0_0_20px_#00f0ff44] text-center"
                          >
                            ÖDE
                          </button>
                        )}
