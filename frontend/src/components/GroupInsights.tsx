...
export const GroupInsights: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/5 rounded-[24px] sm:rounded-[40px] p-4 sm:p-8 md:p-12 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-[#00f0ff]/5 to-[#b026ff]/5 blur-3xl -mr-32 -mt-32 pointer-events-none" />
          ...
          <div className="space-y-6 sm:space-y-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tighter uppercase">İstatistikler</h3>
              ...
              <div className="flex gap-1 bg-white/5 p-1 rounded-2xl overflow-x-auto no-scrollbar">
                {(['Ay', 'Yıl', 'Tümü'] as FilterType[]).map(t => (
                  <button key={t} onClick={() => setFilterType(t)} className={`px-4 sm:px-5 py-2 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterType === t ? 'bg-white text-black shadow-xl' : 'text-slate-400 hover:text-white'}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-12 bg-white/5 p-5 sm:p-10 rounded-[24px] sm:rounded-[40px] border border-white/5">
