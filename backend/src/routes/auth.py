    ...
    async with get_session() as session:
        ...
        try:
            ...
            response.add_cookie(
                ...
                access_token,
                httponly=True,
                secure=os.getenv("SANIC_DEBUG", "false").lower() == "false",
                samesite="Lax",
...
# =============================================================================
# ENDPOINT 6: POST /api/auth/logout — Çıkış Yap
# =============================================================================

@auth_bp.post("/logout")
async def logout(request: Request) -> HTTPResponse:
    """HttpOnly cookie'yi silerek oturumu kapatır."""
    response = sanic_json({"message": "Oturum kapatıldı."})
