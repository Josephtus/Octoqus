...
async def export_expenses(request: Request, group_id: int) -> HTTPResponse:
    ...
    async with get_session() as session:
        ...
        for e in expenses:
            ...
            # Site adresini dinamik olarak al (localhost veya octoqus.com)
            ...
            data.append({
                ...
                "Fatura URL": f"{base_url}{e.bill_photo}" if e.bill_photo else "-"
            })
        ...
        if format_type == "excel":
            ...
            worksheet = workbook.add_worksheet('Harcamalar')
            
            if data:
                headers = list(data[0].keys())
                # Başlıkları yaz (kalın)
                ...
                for col_num, header in enumerate(headers):
                    max_len = max(
                        len(str(header)),
                        max((len(str(row[header])) for row in data), default=0)
                    ) + 2
