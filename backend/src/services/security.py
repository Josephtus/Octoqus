...
def protected(route_fn: Callable) -> Callable:
    ...
    Önce HttpOnly cookie (access_token), yoksa Authorization header kontrol edilir.
    """
    @wraps(route_fn)
    async def wrapper(request: Request, *args: Any, **kwargs: Any) -> Any:
        # 1. Çerez kontrolü (Web App güvenliği için öncelikli)
        raw_token = request.cookies.get("access_token")

        # 2. Başlık kontrolü (API/Mobil desteği için fallback)
        if not raw_token:
        ...
        if not raw_token:
            raise Unauthorized(
                "Oturum açmanız gerekiyor. Token bulunamadı.",
                scheme="Bearer",
            )
